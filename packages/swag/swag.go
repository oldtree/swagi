...
//this file is define and load config swagger description
...
func LoadSwagger(path string) (ok bool, err error) {
	Swag.Description = new(Swagger)
	...
	if fileExt == ".json" {
		...
		err = json.Unmarshal(data, Swag.Description)
		if err != nil {
			log.Info(err)
			ok = false
			return
		...
		err = yaml.Unmarshal(data, Swag.Description)
		if err != nil {
			log.Info(err)
			ok = false
			return
		...
		ok = false
		err = errors.New("error file type")
		return
	}
	ok = true
	err = nil
